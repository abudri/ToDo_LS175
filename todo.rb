...
# returns an error messagse if the name of the list attempted to be submitted is not valid and if so returns a string, otherwise will return nil
def error_for_list_name(name) # for refactoring validations assignment, https://launchschool.com/lessons/9230c94c/assignments/b47401cd
...
post "/lists" do
  ...
  if error = error_for_list_name(list_name) # method call returns a string error message from the method if the list_name passed in is invalid, otherwise it will return nil and this branch of the if statement won't be executed. assign and evaluate at the same time. everything in Ruby is an expression, so the assingment here returns a value just as any other expression, so can test value while at the same time assigning.
